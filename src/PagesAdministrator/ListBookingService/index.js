...
function ListBookingServicesPageComponent(props) {
  ...
  const csvDownload = () => {
     listListBookingServices.data.map((item, index) => {
      return {
        id:
          (request.paging.pageIndex - 1) * request.paging.pageSize + 1 + index,
  ...
  return (
    <Spin className="news-container" spinning={isLoading}>
      <p className="news-breadcrumb">
        {t("Home")} / {t("ServicesTitle")} /{" "}
        <span className="current-breadcrumb">{t("BookingServices")}</span>
      </p>
      ...
      >
        <p>
          <strong>{t("ServiceName")}: </strong> {bookingData.serviceName}
        </p>
        <p>
          <strong>{t("BookingDate")}: </strong> {bookingData.chooseDate}
        </p>
        <p>
          <strong>{t("BookingTime")}: </strong> {bookingData.bookingTime}
        </p>
        <p>
          <strong>{t("Status")}: </strong> {bookingData.status}
        </p>
        <p>
          <strong>{t("CreatedBy")}: </strong> {bookingData.createdBy}
        </p>
        <p>
          <strong>{t("CreatedAt")}: </strong> {bookingData.createdAt}
        </p>
      </Drawer>
    </Spin>
  );
